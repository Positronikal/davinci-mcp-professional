...
class DaVinciMCPServer:
    ...
    async def _call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        ...
        # Project tools
            ...
            return f"Successfully opened project '{name_arg}'" if result else f"Failed to open project '{name_arg}'"
        
        elif name == "create_project":
            name_arg = arguments.get("name", "")
            result = self.resolve_client.create_project(name_arg)
            return f"Successfully created project '{name_arg}'" if result else f"Failed to create project '{name_arg}'"
        ...
        # Timeline tools
            ...
            return f"Successfully created timeline '{name_arg}'" if result else f"Failed to create timeline '{name_arg}'"
        
        elif name == "switch_timeline":
            name_arg = arguments.get("name", "")
            result = self.resolve_client.switch_timeline(name_arg)
            return f"Successfully switched to timeline '{name_arg}'" if result else f"Failed to switch to timeline '{name_arg}'"
        ...
        # Media tools
            ...
            return f"Successfully imported media '{file_path}'" if result else f"Failed to import media '{file_path}'"
